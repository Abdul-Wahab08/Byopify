 ...
 */
function buildByopifyTextLayer({ w, h }: ImageKitUrlOpts) {
  ...
  return `l-text,i-Byopify,fs-${fs},co-FFFFFF,bg-0F172A90,pa-8_12,lx-N14,ly-14,lap-top_right,l-end`;
}
 ...
 */
function buildTrSegment({ w, h, q = 80, f = "auto", crop, watermark = false }: ImageKitUrlOpts) {
  const parts = [];
  ...
  return `${base}:${buildByopifyTextLayer({ w, h })}`;
}
 ...
 */
export function imageKitOptimizedUrl(url: string | null, opts = {}) {
...
/**
 * Same optimizations as {@link imageKitOptimizedUrl} plus Byopify text overlay (for share/download).
