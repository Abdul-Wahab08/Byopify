...
import type { OutletContextProps } from "./OrderSummaryPage";
import { HeadphonesIcon, VideoIcon } from "lucide-react";
import Error from "../components/Error";
import { OrderChatPanelSkeleton } from "../components/LoadingSkeletons";
import { Channel, ChannelHeader, Chat, MessageList, Thread, Window, MessageComposer } from "stream-chat-react";
import "stream-chat-react/dist/css/index.css";

function OrderChat() {
  const { order } = useOutletContext<OutletContextProps>();
  ...
  if (order.status !== "paid") return <p className="text-base-content/60">Complete payment to open support chat.</p>;

  if (!client || !channel) return <OrderChatPanelSkeleton />;
